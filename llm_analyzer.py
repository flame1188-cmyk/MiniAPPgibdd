...
SYSTEM_PROMPT = (
    ...
    "в анализе. Каждый очаг содержит агрегированные причины: нарушения ПДД, "
    "недостатки УДС, состояние проезжей части, факторы режима движения, "
    "технические неисправности ТС (с количеством случаев). Опирайся на эти данные "
    "при анализе причин концентрации и формировании рекомендаций для каждого очага. "
    "Если по какому-то направлению данных нет — не придумывай.\n"
...
SYSTEM_PROMPT_PAID = (
    ...
    "Каждый очаг содержит агрегированные причины (нарушения ПДД, недостатки УДС, "
    "состояние проезжей части, факторы режима движения, технические неисправности "
    "с количеством случаев). Опирайся на них при причинах и рекомендациях. "
    "Не придумывай причины, которых нет в данных очага.\n\n"
...
def _format_dtp_block(
    ...
    cleaned = [v.strip() for v in dtp_fields]
...
def _format_uch_block(
    ...
    Замыкающие пустые поля обрезаются для экономии."""
    cleaned = [v.strip() for v in uch_fields]
    # Обрезаем замыкающие пустые поля
    while cleaned and cleaned[-1].strip() == "":
        cleaned.pop()
