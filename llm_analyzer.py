...
SYSTEM_PROMPT = (
    ...
    "ОБЯЗАТЕЛЬНО используй данные о причинах ДТП в очагах (нарушения ПДД, "
    "недостатки дороги, факторы режима движения, состояние проезжей части, "
    "технические неисправности) — формулируй рекомендации на их основе. "
    "Например: если указан ремонт дороги + нарушение применения временных знаков — "
    "рекомендуй усиление организации движения в зоне ремонта.\n"
...
SYSTEM_PROMPT_PAID = (
    ...
    "3. ПРАВИЛО СРАВНЕНИЯ С АППГ: в КАЖДОМ разделе (не только в «ОБЩАЯ ОЦЕНКА») "
    "ВСЕ приведённые абсолютные числа должны сопровождаться сравнением с предыдущим периодом. "
    "Формат: «X ДТП (было Y, +Z%)» или «X погибших (рост на N%)» или «X (без изменений)». "
    "Если за предыдущий период данных по конкретному срезу нет — укажи «данных АППГ нет». "
    "Пользователь должен видеть по КАЖДОЙ цифре — выросла она или снизилась.\n"
    "4. КОДИРОВКА СТАЖА ВОДИТЕЛЯ: значения 95-99 — это не ошибки, а специальные коды: "
    "стаж 95 = отсутствие прав (или лишён прав), "
    "стаж 96 = отсутствие необходимой категории для управления данным ТС, "
    "стаж 99 = велосипедисты (не водители ТС). "
    "Интерпретируй их корректно, НЕ считай реальным стажем. "
    "При анализе по стажу группируй: 95+96 = «без прав/без категории», 99 = «велосипедисты», "
    "а реальные стажи — в стандартные группы (0-2, 3-5, 6-10, 10+ лет).\n"
    "5. Не повторяй одну и ту же информацию в разных разделах. "
    ...
    "6. Структурируй ответ по разделам с заголовками (без markdown, без эмодзи).\n"
    "7. Пиши на русском языке, профессиональным но понятным стилем.\n"
    "8. Не дублируй раздел «Очаги ДТП» — упомяни их один раз в соответствующем разделе.\n\n"
    ...
    "ОБЯЗАТЕЛЬНО с сравнением с предыдущим периодом (правило 3).\n"
    ...
    "недостатки дороги, факторы режима движения, состояние проезжей части, "
    "технические неисправности) — формулируй рекомендации на их основе. "
    "Например: если в очаге указан ремонт дороги и нарушение применения временных знаков — "
    "рекомендуй корректировку схемы организации движения в зоне ремонта."
    ...
    "Объём ответа: не ограничивай количество символов. "
    "Пиши подробно и развернуто по каждому разделу, приводя все значимые данные, "
    "корреляции и выводы. Приоритет — полнота анализа, точность, конкретика и сравнение с АППГ.\n\n"
    "ДИАЛОГ:\n"
    "Если в сообщениях выше есть история предыдущих вопросов и твоих ответов — "
    "это продолжение диалога с пользователем.\n"
...
def _format_cluster_causes(c: dict[str, Any], top_n: int = 3) -> list[str]:
    """Форматирует блок причин ДТП для одного очага (для LLM-промпта).

    Возвращает список строк. Пустой список если нет причин.
    Каждая категория выводится отдельной строкой только если есть данные.
    """
    lines: list[str] = []

    # Непосредственные нарушения ПДД
    npdd = c.get("npdd_counter", {})
    if npdd:
        parts = [f"{k} ({v})" for k, v in sorted(npdd.items(), key=lambda x: -x[1])[:top_n]]
        lines.append(f"  Нарушения ПДД: {'; '.join(parts)}")

    # Недостатки ТЭС
    ndu = c.get("ndu_counter", {})
    if ndu:
        parts = [f"{k} ({v})" for k, v in sorted(ndu.items(), key=lambda x: -x[1])[:top_n]]
        lines.append(f"  Недостатки дороги: {'; '.join(parts)}")

    # Фактор режима движения
    factor = c.get("factor_counter", {})
    if factor:
        parts = [f"{k} ({v})" for k, v in sorted(factor.items(), key=lambda x: -x[1])[:top_n]]
        lines.append(f"  Факторы движения: {'; '.join(parts)}")

    # Состояние проезжей части
    spch = c.get("spch_counter", {})
    if spch:
        parts = [f"{k} ({v})" for k, v in sorted(spch.items(), key=lambda x: -x[1])[:top_n]]
        lines.append(f"  Состояние дороги: {'; '.join(parts)}")

    # Технические неисправности
    tn = c.get("tn_counter", {})
    if tn:
        parts = [f"{k} ({v})" for k, v in sorted(tn.items(), key=lambda x: -x[1])[:top_n]]
        lines.append(f"  Тех. неисправности: {'; '.join(parts)}")

    # Сопутствующие нарушения (опьянение / лишение) — отдельно, т.к. это агрегирующая категория
    sop = c.get("sop_npdd_counter", {})
    if sop:
        parts = [f"{k} ({v})" for k, v in sorted(sop.items(), key=lambda x: -x[1])[:top_n]]
        lines.append(f"  Сопутствующие (опьянение/лишение): {'; '.join(parts)}")
...
def format_clusters_for_prompt(
    ...
    def format_cluster_block(
        ...
        for i, c in enumerate(sorted_items, 1):
            ...
            # --- Причины ДТП ---
            causes_lines = _format_cluster_causes(c)
            out.extend(causes_lines)
